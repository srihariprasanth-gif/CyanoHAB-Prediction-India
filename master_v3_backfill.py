...
print("🚀 Initializing Master V3 Data Rescue Protocol...")
try:
...
# 2. Load the newly merged V3 dataset
v3_file = 'data/processed/master_hab_dataset_v3.csv'
...
print("⏳ Scanning 2,273 rows and patching missing flow telemetry...")
...
for index, row in df.iterrows():
    if pd.isna(row['Solar_Radiation']) or pd.isna(row['Runoff']):
        ...
        coords = phase2_regional_dams.get(lake)
...
# 4. Save the fully repaired V3 dataset
df.to_csv(v3_file, index=False)
print(f"\n✅ V3 DATA RESCUE COMPLETE! {patched_count} rows successfully patched and secured for Deep Learning.")