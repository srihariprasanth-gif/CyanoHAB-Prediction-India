...
# ==========================================
print("🚀 PHASE 1 (V2.2): Initializing High-Speed Checkpoint Extraction...")
try:
...
# ==========================================
# 3. EXTRACTION ENGINE (16 PARAMETERS)
# ==========================================
...
def extract_v2_data(lake_name, coords, year, month, static_clay, static_ph):
    ...
    try:
        ...
        green = get_val(curr_l8, 'SR_B3', 30)
        ...
        chl_a_proxy = (nir / red) if nir is not None and red is not None and red != 0 else np.nan
        ndti_turbidity = (red - green) / (red + green) if red is not None and green is not None else np.nan
        ndwi_water = (green - nir) / (green + nir) if green is not None and nir is not None else np.nan
        ...
        runoff = get_val(curr_era5, 'runoff', 10000)
...
# ==========================================
# 4. FULL THROTTLE WITH CHECKPOINTING
# ==========================================
target_years = list(range(2016, 2027))
save_dir = 'data/processed/individual_dams'
os.makedirs(save_dir, exist_ok=True)

print(f"⚡ Running API Extraction with Checkpointing...")

for lake_name, coords in phase2_regional_dams.items():
    file_path = os.path.join(save_dir, f'{lake_name}_v2.csv')
    
    # CHECKPOINT LOGIC: Skip if file already exists
    if os.path.exists(file_path):
        print(f"\n⏭️ Skipping {lake_name} - Already secured on hard drive.")
        continue

    print(f"\n🌊 Starting {lake_name}...")
    dam_data = [] # Reset memory for the new dam
    ...
    for yr in target_years:
        for mn in range(1, 13): 
            ...
            while not success and attempts < 3:
                try:
                    row = extract_v2_data(lake_name, coords, yr, mn, static_clay, static_ph)
                    if row:
                        dam_data.append(row)
    ...
    # WRITE TO HARD DRIVE IMMEDIATELY AFTER DAM FINISHES
